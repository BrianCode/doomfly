...
from .common import require_single_blas_thread, OUT, save_json, annotations
...
def run(out=OUT/'conditioning.json'):
    ...
    rows=[]
    for condition in ['paired','backward','frozen','no_dopamine','reset_memory']:
        ...
        if condition=='no_dopamine':pulse_starts=[]
    ...
    qualitative=(all(x is not None for x in suppression) and suppression[0]>suppression[1] and
        ...
        all(r['memory_after_training']['changed_edges']==0 for r in rows if r['condition'] in ['backward','frozen','no_dopamine']) and
